...
module.exports = function(tileLayers, tile, writeData, done) {
  ...
  for (var i = 0; i < layer.features.length; i++) {
    ...
    else if (val.geometry.type === 'MultiLineString' && preserveType[val.properties.highway] && val.properties.layer === undefined) {
  ...
  for (var m = 0; m < bboxes.length; m++) {
    ...
    if (!_.isEqual(firstCoord, endCoord)) {
      ...
      for (var k = 0; k < overlapsFirstPoint.length; k++) {
        ...
        var overlapHighway = highways[overlapPointFirst[4]].highway;
        ...
        if (valueBbox[4] !== overlapPointFirst[4] && (arrayCorrd.indexOf(firstCoord[0]) === -1 || arrayCorrd.indexOf(firstCoord[1]) === -1)) {
          if (turf.inside(firstPoint, highways[overlapPointFirst[4]].buffer)) {
            props.wayB = overlapHighway.properties._osm_way_id;
            firstPoint.properties = props;
            //Check out whether the streets are connected at some point
            var coordinates = valueHighway.geometry.coordinates;
            var valueCoor = _.flatten([coordinates[1], coordinates[2]]);
            var overlapCoor = _.flatten(overlapHighway.geometry.coordinates);
            if (_.intersection(valueCoor, overlapCoor).length < 2) {
              output[valueBbox[4]] = valueHighway;
              output[valueBbox[4]].properties._osmlint = osmlint;
              output[overlapPointFirst[4]] = overlapHighway;
              output[overlapPointFirst[4]].properties._osmlint = osmlint;
              if (valueHighway.properties._osm_way_id > overlapHighway.properties._osm_way_id) {
      ...
      for (var l = 0; l < overlapsEndPoint.length; l++) {
        ...
        var overlapHighway = highways[overlapPointEnd[4]].highway;
        if (valueBbox[4] !== overlapPointEnd[4] && (arrayCorrd.indexOf(endCoord[0]) === -1 || arrayCorrd.indexOf(endCoord[1]) === -1)) {
          if (turf.inside(endPoint, highways[overlapPointEnd[4]].buffer)) {
            ...
            //Check out whether the streets are connected at some point
            var coordinates = valueHighway.geometry.coordinates;
            var valueCoor = _.flatten([coordinates[coordinates.length - 1], coordinates[coordinates.length - 2]]);
            var overlapCoor = _.flatten(overlapHighway.geometry.coordinates);
            if (_.intersection(valueCoor, overlapCoor).length < 2) {
              output[valueBbox[4]] = valueHighway;
              output[valueBbox[4]].properties._osmlint = osmlint;
              output[overlapPointEnd[4]] = overlapHighway;
