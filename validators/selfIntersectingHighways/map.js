...
module.exports = function(tileLayers, tile, writeData, done) {
  ...
  for (var i = 0; i < layer.features.length; i++) {
    ...
    if (
      preserveType[highway.properties.highway] &&
      highway.geometry.type === 'LineString'
    ) {
      if (
        highway.geometry.coordinates.length > 2 &&
        !(
          highway.geometry.coordinates.length.length === 3 &&
          highway.geometry.coordinates[0][0] !==
            highway.geometry.coordinates[2][0] &&
          highway.geometry.coordinates[0][1] !==
            highway.geometry.coordinates[2][1]
        )
      ) {
      ...
      preserveType[highway.properties.highway] &&
      highway.geometry.type === 'MultiLineString'
    ) {
      var arrayWays = flatten(highway);
      for (var f = 0; f < arrayWays.length; f++) {
        if (
          arrayWays[f].geometry.type === 'LineString' &&
          arrayWays[f].geometry.coordinates.length > 2 &&
          !(
            arrayWays[f].geometry.coordinates.length.length === 3 &&
            arrayWays[f].geometry.coordinates[0][0] !==
              arrayWays[f].geometry.coordinates[2][0] &&
            arrayWays[f].geometry.coordinates[0][1] !==
              highway.geometry.coordinates[2][1]
          )
        ) {
      ...
      if (intersect && intersect.features.length > 0) {
        ...
        if (
          ...
          for (var t = 0; t < intersectCoords.length; t++) {
            ...
            intersectPoint.properties._fromWay =
